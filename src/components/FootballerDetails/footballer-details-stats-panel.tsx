...
const FootballerDetailsStatsPanel = ({ footballer }: Props) => {
  ...
  return (
    <div className="mt-2 flex-col shadow-sm">
      <h3 className="ml-2 w-fit rounded-t-md bg-magenta px-2 pb-[0px] pt-1 text-center text-sm text-text lg:ml-6 lg:min-w-[45%] lg:px-4 lg:text-base">
        Season Stats
      </h3>
      <div className="flex w-full items-center gap-1 rounded-md bg-accent5 px-2 pb-1 pt-2 shadow-lg lg:px-4 lg:pb-2 lg:pt-4">
        {seasonStats.map((stat, index) => (
          <React.Fragment key={index}>
            <div className="flex w-8 flex-col items-center justify-center gap-1 whitespace-nowrap lg:w-[68px]">
              <p className="text-xs lg:text-sm">{stat.title}</p>
              <p className="text-xs lg:text-base">{stat.value}</p>
            </div>
